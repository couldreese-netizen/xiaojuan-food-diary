...
if not st.session_state.logged_in:
    ...
    if user_id_param and username_param:
        try:
            user = get_user_by_id(int(user_id_param))
...
# ========== 已登录时显示主页面 ==========
        ...
        if len(filtered_recs) == 0:
            ...
            for rec in filtered_recs:
                ...
                if is_my:
                    ...
                    user_avatar = rec_user.get("avatar") if rec_user else None
                type_icon = "🍱" if rec.get("food_type", "外卖吃啥") == "外卖吃啥" else "🍽️"
                type_text = rec.get("food_type", "外卖吃啥")
                # 头像行
                avatar_col, name_col = st.columns([1, 5])
                with avatar_col:
                ...
                with name_col:
                    if is_my:
                        st.markdown(f"**🐯 我**  {type_icon} {type_text}")
                    elif is_friend:
                        st.markdown(f"**👥 {user_label}**  {type_icon} {type_text}")
                    else:
                        st.markdown(f"**🍜 {user_label}**  {type_icon} {type_text}")
                ...
                like_users = []
                for uid in rec.get("likes", []):
                    u = get_user_by_id(uid)
                    if u:
                        like_users.append(get_user_display_name(u))
                like_tooltip = "👍 点赞的人：" + ", ".join(like_users) if like_users else "暂无点赞"
                col_like, col_empty = st.columns([1, 5])
                with col_like:
                    button_label = f"❤️ {like_count}" if liked else f"🤍 {like_count}"
                    if st.button(button_label, key=f"like_{rec['id']}", help=like_tooltip):
        ...
        if "editing_rec" in st.session_state:
            ...
            with st.expander("✏️ 编辑美食日记", expanded=True):
                ...
                with st.form("edit_food_form"):
                    ...
                    edit_food_type = st.radio("选择类型", type_options, horizontal=True, index=type_index)
                    st.markdown("### 👥 和谁一起吃？")
                    ...
                    ate_with = st.multiselect("选择和谁一起吃的", ate_with_options, default=current_friends, help="记录和谁一起分享美食")
                    st.markdown("### 📸 美食照片")
                    ...
                    new_photo = None
                    if change_photo:
                        new_photo = st.file_uploader("上传新照片（可选）", type=["jpg", "jpeg", "png"], key=f"edit_photo_{rec_to_edit['id']}")
                        if new_photo:
                            preview_img = Image.open(new_photo)
                            preview_img = ImageOps.exif_transpose(preview_img)
                            st.image(preview_img, width=150, caption="新照片预览")
                    st.markdown("### 💬 美食评价")
                    new_reason = st.text_area("推荐理由", value=rec_to_edit.get("reason", ""), height=100, key=f"edit_reason_{rec_to_edit['id']}")
                    new_rating = st.slider("评分", 1, 5, value=rec_to_edit.get("rating", 4), key=f"edit_rating_{rec_to_edit['id']}")
                    new_tags = st.text_input("标签", value=rec_to_edit.get("tags", ""), key=f"edit_tags_{rec_to_edit['id']}")
                    col1, col2 = st.columns(2)
                    with col1:
                    ...
                    if submit_edit:
                        updates = {
                            ...
                            "ate_with": ", ".join([a for a in ate_with if a != "🐯 独自一人"]) if ate_with else "",
                            "reason": new_reason,
                            "rating": new_rating,
                            "tags": new_tags if new_tags else "美食"
                        }
                        if new_photo:
                            image_path = save_image(new_photo, st.session_state.user_id, rec_to_edit["restaurant"])
    ...
    # ========== 记录今日美食 ==========
        ...
        if "food_cache" not in st.session_state:
            st.session_state.food_cache = {
                "city": "", "district": "", "restaurant": "", "dish": "",
                "rating": 4, "price": 50, "food_type": "🍱 外卖吃啥",
                "ate_with": [], "reason": "", "tags": "", "photo": None
            }
        friends = get_friends(st.session_state.user_id)
        ...
        with st.form("add_food"):
            ...
            with col2:
                price = st.number_input("💰 人均价格（元）", min_value=0, max_value=9999, value=cache["price"], step=10)
            st.markdown("### 🍱 美食分类")
            food_type = st.radio("选择类型", ["🍱 外卖吃啥", "🍽️ 奢侈一把"], horizontal=True, index=0 if cache["food_type"] == "🍱 外卖吃啥" else 1)
            st.markdown("### 👥 和谁一起吃？")
            friend_names = [get_user_display_name(f) for f in friends]
            ate_with_options = ["🐯 独自一人"] + friend_names
            default_friends = [f for f in cache["ate_with"] if f in ate_with_options]
            ate_with = st.multiselect("选择和谁一起吃的", ate_with_options, default=default_friends)
            st.markdown("### 💬 美食评价")
            ...
            st.markdown("### 📸 美食照片")
            uploaded_photo = st.file_uploader("上传美食照片", type=["jpg", "jpeg", "png"], key="photo_uploader")
            if uploaded_photo:
            ...
            if submitted:
                if not restaurant:
                    st.error("请填写店名")
                    st.session_state.food_cache = {"city": city, "district": district, "restaurant": restaurant, "dish": dish, "rating": rating, "price": price, "food_type": food_type, "ate_with": ate_with, "reason": reason, "tags": tags, "photo": uploaded_photo}
                elif not city:
                    st.error("请填写城市")
                    st.session_state.food_cache = {"city": city, "district": district, "restaurant": restaurant, "dish": dish, "rating": rating, "price": price, "food_type": food_type, "ate_with": ate_with, "reason": reason, "tags": tags, "photo": uploaded_photo}
                elif not reason:
                    ...
                    new_rec = {
                        ...
                        "city": city, "district": district or "未填写", "restaurant": restaurant,
                        "dish": dish or "推荐菜品", "rating": rating, "price": price,
                        "reason": reason, "tags": tags or "美食",
                        "date": now_beijing.strftime("%Y-%m-%d %H:%M"), "image_path": image_path,
                        "ate_with": ate_with_str, "food_type": type_value, "likes": []
                    }
                    add_recommendation(new_rec)
                    st.session_state.food_cache = {"city": "", "district": "", "restaurant": "", "dish": "", "rating": 4, "price": 50, "food_type": "🍱 外卖吃啥", "ate_with": [], "reason": "", "tags": "", "photo": None}
    ...
    # ========== 个人中心 ==========
        ...
        if st.session_state.user_id == 1:
            with st.expander("🔧 管理员面板（仅小卷可见）"):
                ...
                if len(users) == 0:
                    ...
                    user_data = []
                    for u in users:
                        user_data.append({"ID": u["id"], "用户名": u["username"], "昵称": u.get("nickname", u["username"]), "邀请码": u["invite_code"], "剩余邀请": u.get("remaining_invites", "无限制" if u["id"] == 1 else "10")})
                    st.table(user_data)
                ...
                with col2:
                    st.metric("总美食日记", len(get_all_recommendations()))
                with col3:
                    total_likes = sum(len(r.get("likes", [])) for r in get_all_recommendations())
            ...
            for rec in liked_recs_sorted:
                ...
                with col_info:
                    ...
                    with avatar_col:
                        if user_avatar and os.path.exists(user_avatar):
                            avatar_img = Image.open(user_avatar)
                            st.image(avatar_img, width=30)
                        else:
                            st.markdown("🫂")
                    with name_col:
                        st.markdown(f"**{get_user_display_name(rec_user)}** 推荐")
                    type_icon = "🍱" if rec.get("food_type", "外卖吃啥") == "外卖吃啥" else "🍽️"
                    st.markdown(f"""
                    **{rec['restaurant']}**  {type_icon}
        ...
        with col2:
            ...
            new_avatar = st.file_uploader("点击选择图片", type=["jpg", "jpeg", "png"], key="edit_avatar", label_visibility="collapsed")
            if new_avatar:
                preview_img = Image.open(new_avatar)
                preview_img = ImageOps.exif_transpose(preview_img)
                st.image(preview_img, width=80)
                st.caption("预览（上传后自动裁剪为正方形）")
        new_nickname = st.text_input("🍜 昵称", value=current_user.get("nickname", current_user["username"]), key="edit_nickname")
        new_bio = st.text_area("📝 一句话介绍自己", value=current_user.get("bio", ""), key="edit_bio", height=80)
